...
public class SyntaxAnalyzer {
    ...
    public void run() {
        ...
        int index = 0;
        while (true){
            Action action = lrTable.getAction(statusStack.peek(),tokens.get(index));
            switch (action.getKind()){
                ...
                case Shift ->{
                    callWhenInShift(statusStack.peek(),tokens.get(index));
                    symbolStack.push(new Symbol(tokens.get(index)));
                    index++;
                ...
                case Error -> {
                    throw new RuntimeException("归约失败");
                }
            }
        }
    }
